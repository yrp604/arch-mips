...
static ExprId SetRegisterOrNop(LowLevelILFunction& il, size_t size, uint32_t reg, ExprId expr)
{
	if (reg == REG_ZERO)
		return il.Nop();
	else
		return il.SetRegister(size, reg, expr);
}


static void ConditionExecute(
    LowLevelILFunction& il, ExprId cond, ExprId trueCase, ExprId falseCase = INVALID_EXPRID)
{
...
static size_t GetILOperandMemoryAddress(
    LowLevelILFunction& il, InstructionOperand& operand, size_t addrSize)
{
	size_t offset = 0;
	...
	if (operand.operandClass == MEM_IMM)
	{
		if (operand.immediate >= 0x80000000)
			offset = il.Sub(addrSize, il.Register(addrSize, operand.reg),
			    il.Const(addrSize, -(int32_t)operand.immediate));
		else
			offset = il.Add(
			    addrSize, il.Register(addrSize, operand.reg), il.Const(addrSize, operand.immediate));
	}
	else if (operand.operandClass == MEM_REG)
	{
		if (operand.immediate >= 0x80000000)
			...
			offset = il.Add(
			    addrSize, il.Register(addrSize, operand.reg), il.Register(addrSize, operand.immediate));
	}
	return offset;
}


static size_t ReadILOperand(LowLevelILFunction& il, const Instruction& instr, size_t i,
    size_t registerSize, size_t addrSize = SIZE_MAX, bool isAddress = false)
{
	...
	case MEM_IMM:
		return il.Operand(
		    i - 1, il.Load(addrSize, GetILOperandMemoryAddress(il, operand, registerSize)));
	default:
		if (operand.reg == REG_ZERO)
...
static size_t WriteILOperand(
    LowLevelILFunction& il, Instruction& instr, size_t i, size_t addrSize, size_t value)
{
	InstructionOperand& operand = instr.operands[i - 1];
	switch (operand.operandClass)
	{
	...
	case MEM_REG:
		return il.Operand(
		    i - 1, il.Store(addrSize, GetILOperandMemoryAddress(il, operand, addrSize), value));
	default:
...
ExprId GetConditionForInstruction(LowLevelILFunction& il, Instruction& instr, size_t registerSize)
{
	...
	case MIPS_BEQ:
	case MIPS_BEQL:
		return il.CompareEqual(registerSize, ReadILOperand(il, instr, 1, registerSize),
		    ReadILOperand(il, instr, 2, registerSize));
	case MIPS_BNE:
	case MIPS_BNEL:
		return il.CompareNotEqual(registerSize, ReadILOperand(il, instr, 1, registerSize),
		    ReadILOperand(il, instr, 2, registerSize));
	case MIPS_BEQZ:
		return il.CompareEqual(
		    registerSize, ReadILOperand(il, instr, 1, registerSize), il.Const(registerSize, 0));
	case MIPS_BGEZ:
	case MIPS_BGEZL:
	case MIPS_BGEZAL:
	...
	case MIPS_BGTZ:
	case MIPS_BGTZL:
		return il.CompareSignedGreaterThan(
		    registerSize, ReadILOperand(il, instr, 1, registerSize), il.Const(registerSize, 0));
	case MIPS_BLEZ:
	case MIPS_BLEZL:
		return il.CompareSignedLessEqual(
		    registerSize, ReadILOperand(il, instr, 1, registerSize), il.Const(registerSize, 0));
	case MIPS_BLTZ:
	case MIPS_BLTZL:
	case MIPS_BLTZAL:
		return il.CompareSignedLessThan(
		    registerSize, ReadILOperand(il, instr, 1, registerSize), il.Const(registerSize, 0));
	case MIPS_BC1F:
	case MIPS_BC1FL:
    ...
    Architecture* arch, uint64_t addr, LowLevelILFunction& il, Instruction& instr, size_t addrSize)
{
	...
	case MIPS_ADDIU:
		if (op2.reg == REG_ZERO)
			il.AddInstruction(
			    SetRegisterOrNop(il, 4, op1.reg, ReadILOperand(il, instr, 3, registerSize)));
		else
			il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
			    il.Add(4, ReadILOperand(il, instr, 2, registerSize),
	...
	case MIPS_DADDIU:
		if (op2.reg == REG_ZERO)
			il.AddInstruction(
			    SetRegisterOrNop(il, 8, op1.reg, ReadILOperand(il, instr, 3, registerSize)));
		else
			il.AddInstruction(SetRegisterOrNop(il, 8, op1.reg,
			    il.Add(8, ReadILOperand(il, instr, 2, registerSize),
			        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_SUB:
	case MIPS_SUBU:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Sub(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_AND:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.And(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_ANDI:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
	...
	case MIPS_DIV:
		il.AddInstruction(il.SetRegister(4, REG_LO,
		    il.DivSigned(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize))));
		il.AddInstruction(il.SetRegister(4, REG_HI,
		    il.ModSigned(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize))));
		break;
	case MIPS_DIVU:
		il.AddInstruction(il.SetRegister(4, REG_LO,
		    il.DivUnsigned(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize))));
		il.AddInstruction(il.SetRegister(4, REG_HI,
		    il.ModUnsigned(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize))));
		break;
	case MIPS_MUL:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Mult(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_XOR:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Xor(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_XORI:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
	...
	case MIPS_BNEL:
		ConditionalJump(arch, il, GetConditionForInstruction(il, instr, registerSize), addrSize,
		    op3.immediate, addr + 8);
		return false;
	...
	case MIPS_BC1FL:
		if (op1.operandClass == FLAG)
			...
			ConditionalJump(
			    arch, il, il.Not(0, il.Flag(FPCCREG_FCC0)), addrSize, op1.immediate, addr + 8);
		return false;
	...
	case MIPS_BLTZAL:
		il.AddInstruction(
		    il.If(GetConditionForInstruction(il, instr, registerSize), trueCode, falseCode));
		il.MarkLabel(trueCode);
	...
	case MIPS_CLO:
		...
		il.MarkLabel(trueCode);
		il.AddInstruction(
		    il.SetRegister(1, LLIL_TEMP(0), il.Add(1, il.Const(1, 1), il.Register(1, LLIL_TEMP(0)))));
	...
	case MIPS_LBUX:
	case MIPS_LBU:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.ZeroExtend(4, ReadILOperand(il, instr, 2, registerSize, 1))));
		break;
	case MIPS_LB:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.SignExtend(4, ReadILOperand(il, instr, 2, registerSize, 1))));
		break;
	case MIPS_MFHI:
	...
	case MIPS_MFC0:
		...
		break;
	case MIPS_MFC1:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, il.Register(4, op2.reg)));
		break;
	case MIPS_MFC2:
	...
	case MIPS_MOVN:
		il.AddInstruction(
		    il.If(il.CompareNotEqual(4, ReadILOperand(il, instr, 3, registerSize), il.Const(4, 0)),
		        trueCode, falseCode));
		il.MarkLabel(trueCode);
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, ReadILOperand(il, instr, 2, registerSize)));
		il.MarkLabel(falseCode);
	...
	case MIPS_MOVZ:
		il.AddInstruction(
		    il.If(il.CompareEqual(4, ReadILOperand(il, instr, 3, registerSize), il.Const(4, 0)),
		        trueCode, falseCode));
		il.MarkLabel(trueCode);
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg, ReadILOperand(il, instr, 2, registerSize)));
		il.MarkLabel(falseCode);
	...
	case MIPS_MSUB:
		...
		il.AddInstruction(il.SetRegister(8, LLIL_TEMP(0),
		    il.Or(8, il.ShiftLeft(8, il.Register(4, REG_HI), il.Const(1, 32)),
		        il.ZeroExtend(8, il.Register(4, REG_LO)))));
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
		    il.Sub(8, il.Register(8, LLIL_TEMP(0)),
		        il.MultDoublePrecSigned(4, ReadILOperand(il, instr, 1, registerSize),
		            ReadILOperand(il, instr, 2, registerSize)))));
		break;
	case MIPS_MSUBU:
		//(HI,LO) = (HI,LO) - (GPR[rs] x GPR[rt])
		...
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
		    il.Sub(8, il.Register(8, LLIL_TEMP(0)),
		        il.MultDoublePrecUnsigned(8, ReadILOperand(il, instr, 1, registerSize),
		            ReadILOperand(il, instr, 2, registerSize)))));
		break;
	case MIPS_MULT:
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
		    il.MultDoublePrecSigned(8, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize))));
		break;
	case MIPS_MULTU:
		il.AddInstruction(il.SetRegisterSplit(4, REG_HI, REG_LO,
	...
	case MIPS_NEGU:
		il.AddInstruction(
		    SetRegisterOrNop(il, 4, op1.reg, il.Neg(4, ReadILOperand(il, instr, 2, registerSize))));
		break;
	case MIPS_NOT:
		il.AddInstruction(
		    SetRegisterOrNop(il, 4, op1.reg, il.Not(4, ReadILOperand(il, instr, 2, registerSize))));
		break;
	case MIPS_NOR:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Not(4, il.Or(4, ReadILOperand(il, instr, 2, registerSize),
		                  ReadILOperand(il, instr, 3, registerSize)))));
		break;
	case MIPS_OR:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Or(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_ORI:
		if (op2.reg == REG_ZERO)
			il.AddInstruction(
			    il.SetRegister(4, op1.reg, il.Operand(1, il.Const(4, 0x0000ffff & op3.immediate))));
		else
			il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
			    il.Or(4, ReadILOperand(il, instr, 2, registerSize),
			        il.Operand(1, il.Const(4, 0x0000ffff & op3.immediate)))));
	...
	case MIPS_SWC3:
		...
		break;
	case MIPS_SWL:
		il.AddInstruction(il.Store(2, GetILOperandMemoryAddress(il, op2, addrSize),
		    il.LogicalShiftRight(4, ReadILOperand(il, instr, 1, registerSize), il.Const(1, 16))));
		break;
	case MIPS_SWR:
		il.AddInstruction(
		    il.Store(2, il.Sub(4, GetILOperandMemoryAddress(il, op2, addrSize), il.Const(4, 1)),
	...
	case MIPS_EXT:
		...
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.And(4, il.Const(4, (1 << op4.immediate) - 1),
		        il.ShiftLeft(
		            4, ReadILOperand(il, instr, 2, registerSize), il.Const(1, op3.immediate)))));
		break;
	case MIPS_INS:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.Or(4,
		        il.And(4, il.Const(4, ((1 << op4.immediate) - 1) << op3.immediate),
		            ReadILOperand(il, instr, 1, registerSize)),
		        il.And(4, il.Const(4, (1 << op4.immediate) - 1),
		            ReadILOperand(il, instr, 2, registerSize)))));
		break;
	...
	case MIPS_SRAV:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.ArithShiftRight(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_SLT:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4, il.CompareSignedLessThan(4, ReadILOperand(il, instr, 2, registerSize),
		                        ReadILOperand(il, instr, 3, registerSize)))));
		break;
	case MIPS_SLTI:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4, il.CompareSignedLessThan(4, ReadILOperand(il, instr, 2, registerSize),
		                        il.Const(4, op3.immediate)))));
		break;
	case MIPS_SLTIU:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.BoolToInt(4, il.CompareUnsignedLessThan(4, ReadILOperand(il, instr, 2, registerSize),
		                        il.Const(4, op3.immediate)))));
		break;
	case MIPS_SLTU:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
	...
	case MIPS_SLLV:
		...
		break;
	case MIPS_SB:
		il.AddInstruction(il.Store(1, GetILOperandMemoryAddress(il, op2, addrSize),
		    il.LowPart(1, ReadILOperand(il, instr, 1, registerSize))));
		break;
	case MIPS_TRAP:
	...
	case MIPS_TEQ:
		ConditionExecute(il,
		    il.CompareEqual(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize)),
		    il.Trap(0));
		break;
	case MIPS_TNE:
	case MIPS_TNEI:
		ConditionExecute(il,
		    il.CompareNotEqual(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize)),
		    il.Trap(0));
		break;
	case MIPS_TGE:
	case MIPS_TGEI:
		ConditionExecute(il,
		    il.CompareSignedGreaterEqual(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize)),
		    il.Trap(0));
		break;
	case MIPS_TGEIU:
	case MIPS_TGEU:
		ConditionExecute(il,
		    il.CompareUnsignedGreaterEqual(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize)),
		    il.Trap(0));
		break;
	case MIPS_TLT:
	case MIPS_TLTI:
		ConditionExecute(il,
		    il.CompareSignedLessThan(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize)),
		    il.Trap(0));
		break;
	case MIPS_TLTIU:
	case MIPS_TLTU:
		ConditionExecute(il,
		    il.CompareUnsignedLessThan(4, ReadILOperand(il, instr, 1, registerSize),
		        ReadILOperand(il, instr, 2, registerSize)),
		    il.Trap(0));
		break;
	case MIPS_LH:
	...
	case MIPS_LHI:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.SignExtend(4, ReadILOperand(il, instr, 2, registerSize, 2))));
		break;
	case MIPS_LHU:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.ZeroExtend(4, ReadILOperand(il, instr, 2, registerSize, 2))));
		break;
	case MIPS_LWL:
	case MIPS_LWR:
	...
	case MIPS_SEB:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.SignExtend(4, il.LowPart(1, ReadILOperand(il, instr, 2, registerSize)))));
		break;
	case MIPS_SEH:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.SignExtend(4, il.LowPart(2, ReadILOperand(il, instr, 2, registerSize)))));
		break;
	case MIPS_SH:
		il.AddInstruction(il.Store(2, GetILOperandMemoryAddress(il, op2, addrSize),
		    il.LowPart(2, ReadILOperand(il, instr, 1, registerSize))));
		break;
	case MIPS_SRL:
	case MIPS_SRLV:
		il.AddInstruction(SetRegisterOrNop(il, 4, op1.reg,
		    il.LogicalShiftRight(4, ReadILOperand(il, instr, 2, registerSize),
		        ReadILOperand(il, instr, 3, registerSize))));
		break;
	case MIPS_SSNOP:
	case MIPS_NOP:
	...
	case MIPS_ADD_S:
		il.AddInstruction(il.SetRegister(
		    4, op1.reg, il.FloatAdd(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		break;
	case MIPS_ADD_D:
	...
	case MIPS_SUB_S:
		il.AddInstruction(il.SetRegister(
		    4, op1.reg, il.FloatSub(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		break;
	case MIPS_SUB_D:
		il.AddInstruction(il.SetRegisterSplit(4, op1.reg | 1, op1.reg & (~1),
	...
	case MIPS_MUL_S:
		il.AddInstruction(il.SetRegister(
		    4, op1.reg, il.FloatMult(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		break;
	case MIPS_MUL_D:
		il.AddInstruction(il.SetRegisterSplit(4, op1.reg | 1, op1.reg & (~1),
	...
	case MIPS_DIV_S:
		il.AddInstruction(il.SetRegister(
		    4, op1.reg, il.FloatDiv(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		break;
	case MIPS_DIV_D:
		il.AddInstruction(il.SetRegisterSplit(4, op1.reg | 1, op1.reg & (~1),
	...
	case MIPS_CVT_S_W:
		il.AddInstruction(
		    il.SetRegister(4, op1.reg, il.IntToFloat(4, ReadILOperand(il, instr, 2, registerSize))));
		break;
	case MIPS_CVT_D_W:
		il.AddInstruction(il.SetRegisterSplit(4, op1.reg | 1, op1.reg & (~1),
	...
	case MIPS_CVT_W_D:
		il.AddInstruction(SetRegisterOrNop(
		    il, 4, op1.reg, il.FloatToInt(4, il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)))));
		break;
	case MIPS_CVT_D_S:
		il.AddInstruction(il.SetRegisterSplit(
		    4, op1.reg | 1, op1.reg & (~1), il.FloatConvert(8, il.Register(4, op2.reg))));
		break;
	case MIPS_CVT_S_D:
		il.AddInstruction(il.SetRegister(
		    4, op1.reg, il.FloatConvert(4, il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)))));
		break;
	case MIPS_C_EQ_S:
		if (op1.operandClass == FLAG)
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareEqual(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(
			    FPCCREG_FCC0, il.FloatCompareEqual(4, il.Register(4, op1.reg), il.Register(4, op2.reg))));
		}
		break;
	case MIPS_C_EQ_D:
		if (op1.operandClass == FLAG)
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareEqual(8, il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)),
			                 il.RegisterSplit(4, op3.reg | 1, op3.reg & (~1)))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(
			    FPCCREG_FCC0, il.FloatCompareEqual(8, il.RegisterSplit(4, op1.reg | 1, op1.reg & (~1)),
	...
	case MIPS_C_LE_S:
		...
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareLessEqual(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(FPCCREG_FCC0,
			    il.FloatCompareLessEqual(4, il.Register(4, op1.reg), il.Register(4, op2.reg))));
		}
		break;
	case MIPS_C_LE_D:
		if (op1.operandClass == FLAG)
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareLessEqual(8, il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)),
			                 il.RegisterSplit(4, op3.reg | 1, op3.reg & (~1)))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(FPCCREG_FCC0,
			    il.FloatCompareLessEqual(8, il.RegisterSplit(4, op1.reg | 1, op1.reg & (~1)),
			        il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)))));
		}
		break;
	case MIPS_C_LT_S:
		...
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareLessThan(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(FPCCREG_FCC0,
			    il.FloatCompareLessThan(4, il.Register(4, op1.reg), il.Register(4, op2.reg))));
		}
		break;
	case MIPS_C_LT_D:
		if (op1.operandClass == FLAG)
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareLessThan(8, il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)),
			                 il.RegisterSplit(4, op3.reg | 1, op3.reg & (~1)))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(
			    FPCCREG_FCC0, il.FloatCompareLessThan(8, il.RegisterSplit(4, op1.reg | 1, op1.reg & (~1)),
			                      il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)))));
		}
		break;
	case MIPS_C_UN_S:
		...
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareUnordered(4, il.Register(4, op2.reg), il.Register(4, op3.reg))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(FPCCREG_FCC0,
			    il.FloatCompareUnordered(4, il.Register(4, op1.reg), il.Register(4, op2.reg))));
		}
		break;
	case MIPS_C_UN_D:
		if (op1.operandClass == FLAG)
		{
			il.AddInstruction(il.SetFlag(
			    op1.reg, il.FloatCompareUnordered(8, il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)),
			                 il.RegisterSplit(4, op3.reg | 1, op3.reg & (~1)))));
		}
		else
		{
			il.AddInstruction(il.SetFlag(FPCCREG_FCC0,
			    il.FloatCompareUnordered(8, il.RegisterSplit(4, op1.reg | 1, op1.reg & (~1)),
			        il.RegisterSplit(4, op2.reg | 1, op2.reg & (~1)))));
		}
		break;
	case MIPS_ADDR:
